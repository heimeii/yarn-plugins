...
export type Algorithm = (project: Project, patterns: Array<string>, opts: {
    ...
    resolvedDedupe: Array<{
        descriptor: Descriptor;
        currentPackage: Package;
        updatedPackage: Package;
    }>;
    unresolvedDedupe: Array<{
        descriptor: Descriptor;
        package: Package;
    }>;
}>;
...
const DEDUPE_ALGORITHMS: Record<Strategy, Algorithm> = {
    highest: async (project, patterns, { resolver, fetcher, resolveOptions, fetchOptions }) => {
        ...
        const resolvedDedupe: Array<{
            descriptor: Descriptor;
            currentPackage: Package;
            updatedPackage: Package;
        }> = [];
        let unresolvedDedupe: Array<{
            descriptor: Descriptor;
            package: Package;
        }> = [];

        await Promise.all(Array.from(project.storedDescriptors.values()).map(async descriptor => {
            ...
            if (updatedResolution !== currentResolution) {
                resolvedDedupe.push({ descriptor, currentPackage, updatedPackage });
            } else {
                if (references.filter(item => item.match(/^npm:/)).length > 1) {
                    unresolvedDedupe.push({ descriptor, package: currentPackage });
                }
            }
        }));
...
export async function dedupe(project: Project, { strategy, patterns, cache, report }: DedupeOptions) {
    ...
    return await report.startTimerPromise(`Start Deduplication`, async () => {
        ...
        let unresolvedDedupedCount = 0;
        const unresolvedPackages = new Map<string, Record<string, string[]>>();
        unresolvedDedupe.forEach(({ descriptor, package: p }) => {
            const packageName = `${descriptor.scope ? `@${descriptor.scope}/` : ''}${descriptor.name}`;
            const o = unresolvedPackages.get(packageName) || {};
            const versions = o[p.locatorHash] = o[p.locatorHash] ?? [];
            versions.push(descriptor.range);
            unresolvedPackages.set(packageName, o);
        });
        report.startTimerSync('Unresolved Packages:', () => {
            unresolvedPackages.forEach((versions, packageName) => {
                unresolvedDedupedCount++;
                report.reportInfo(
                    MessageName.UNNAMED,
        ...
        return {
            resolved: resolvedDedupedCount,
            unresolved: unresolvedDedupedCount,
        };
    });
}
